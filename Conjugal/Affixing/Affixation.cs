...
public readonly ref partial struct Affixation {
    ...
    public AffixFlavor AffixFlavor { get; internal init; }
    ...
    private Span3 GetParts() {
        return AffixFlavor switch {
            ...
            AffixFlavor.Transfix  => throw AffixFlavor.NotImplementedException(),
            AffixFlavor.Disfix    => throw AffixFlavor.NotImplementedException(),
    ...
    public string Describe() {
        return $"{nameof(Stem)}: '{Stem.ToString()}', {AffixFlavor.ToString()}: '{BoundMorpheme.ToString()}', {nameof(Joiner)}: '{Joiner.ToString()}', {nameof(InsertionPoint)}: '{InsertionPoint.ToString()}'";
    }
